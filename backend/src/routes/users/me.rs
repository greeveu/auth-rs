...
pub async fn get_current_user(db: Connection<AuthRsDatabase>, req_entity: AuthEntity) -> Json<HttpResponse<UserMinimal>> {
...
pub async fn get_current_user_plain(db: Connection<AuthRsDatabase>, req_entity: AuthEntity) -> Option<Json<UserMinimal>> {
    if req_entity.is_token() && (!req_entity.token.clone().unwrap().check_scope(OAuthScope::Users(ScopeActions::Read)) || req_entity.token.clone().unwrap().check_scope(OAuthScope::Users(ScopeActions::All))) {
        return None;
    }
