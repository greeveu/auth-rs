...
pub async fn disconnect(db: Connection<AuthRsDatabase>, req_entity: AuthEntity, id: &str) -> Json<HttpResponse<OAuthApplicationMinimal>> {
    if req_entity.is_token() && (!req_entity.token.clone().unwrap().check_scope(OAuthScope::Connections(ScopeActions::Delete)) || req_entity.token.clone().unwrap().check_scope(OAuthScope::Connections(ScopeActions::All))) {
